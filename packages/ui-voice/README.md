# `@flowingspring/dsh-client-ui-voice`

English | [中文](README.zh.md)

Browser conversation surface for the voice profile. The first microphone-button press attaches Voice Mode to the current durable Session; once connected, the same button toggles microphone mute with a red slashed icon. Muting disables the microphone track, commits already-sent partial speech, and stops later PCM upload while preserving the `/voice` WebSocket, typed turns, and spoken playback. Holding the microphone button for 400 ms starts a strict push-to-talk utterance; releasing it commits exactly that utterance, while cancelling or leaving the page safely releases capture. A push-to-talk utterance temporarily restores the input mode that was active before the hold; an initially disconnected session stays connected but muted after submission so its spoken reply can arrive. The root overlay retains the separate terminal End action. That source Session is the recoverable Voice conversation window; only creating another ordinary Session creates another Voice conversation identity. A root-owned `/voice` WebSocket keeps capture and playback alive when the user visits a delegated task. Capture starts only after the server's `ready` frame confirms the provider session and audio settings. The package captures echo-cancelled audio, resamples it to 16 kHz PCM, schedules returned MP3 audio, and stops queued playback immediately when a new transcription starts. A provider error or terminal close releases the WebSocket, microphone tracks, playback sources, and audio contexts.
