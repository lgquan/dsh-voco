...
describe('local ONNX model seam', () => {
  it('maps one model root to all platform-neutral asset paths', () => {
    const layout = createModelLayout('D:/dsh-vox/speech/models')
    expect(layout.vad.replaceAll('\\', '/')).toBe('D:/dsh-vox/speech/models/vad/silero_vad.onnx')
  ...
  it('reports setup instructions when assets are absent', async () => {
    await expect(assertModelsInstalled(createModelLayout('D:/missing-dsh-vox-models')))
