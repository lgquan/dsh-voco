...
Speech-shell transcriptions enter the Voice Session Agent as human messages. Frontend-agent work enters the fixed background task Agent as a natural-language human message because the provider model selected its wording. Only that task Agent receives `send_voice_message`; the server binds that tool to the active task automatically, so the model neither sees nor submits an internal task id, and the voice provider receives no dsh business-tool schema.
