...
Speech-shell transcriptions enter the Voice Session Agent as human messages. Frontend-agent work enters a fresh task Agent as a human delegation envelope because the provider model selected its wording. Only that task Agent receives `send_voice_message`; the Duplex Agent still receives no dsh business-tool schema.
