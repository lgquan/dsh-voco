...
Delegation envelopes, ordinary task execution, and `send_voice_message` calls consume task-model tokens. The independent final-result rewrite is one auxiliary call to the selected default model. Direct frontend conversation and local ASR/TTS stay outside the task-model request.
